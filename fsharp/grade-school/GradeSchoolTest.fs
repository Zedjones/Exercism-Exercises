...
let studentsToSchool (students: List<string * int>): School =
    let schoolFolder school (name, grade) = add name grade school
...
let ``Adding more student adds them to the sorted roster`` () =
    let school =
        studentsToSchool [ ("Blair", 2)
                           ("James", 2)
                           ("Paul", 2) ]

    roster school
    |> should equal [ "Blair"; "James"; "Paul" ]

[<Fact>]
let ``Adding students to different grades adds them to the same sorted roster`` () =
    let school =
        studentsToSchool [ ("Chelsea", 3)
                           ("Logan", 7) ]

    roster school
    |> should equal [ "Chelsea"; "Logan" ]
...
let ``Grade returns the students in that grade in alphabetical order`` () =
    let school =
        studentsToSchool [ ("Franklin", 5)
                           ("Bradley", 5)
                           ("Jeff", 1) ]

    grade 5 school
    |> should equal [ "Bradley"; "Franklin" ]
